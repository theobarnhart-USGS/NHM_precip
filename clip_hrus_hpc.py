...
def clip_raster(df):
    ...
    cmd = "gdalwarp -cutline %s -cwhere hru_id_reg=%s -crop_to_cutline -overwrite %s %s"%(cutline,
...
file = '~/projects/NHM_precipitation/data/hru_%s_clean.shp'%reg
inpath = './hrap_grid_AEA_idx_sm.img'
runClip(file,inpath=inpath) # crop the files
missing = check(file,inpath=inpath) # figure out whats missing
missing.to_pickle('~/projects/NHM_precipitation/data/missing_region_%s.pcl'%reg) # save the file
